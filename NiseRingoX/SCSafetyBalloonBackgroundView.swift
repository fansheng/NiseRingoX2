 ...
 */
    ...
    required init?(coder: NSCoder) {
        ...
        //self.textStorage=NSTextStorage(string: "Here's to the crazy ones, the misfits, the rebels, the troublemakers, the round pegs in the square holes, the ones who see things differently.")
        super.init(coder: coder)
        //self.textContainer?.size=NSSize(width: cgImage.width, height: cgImage.height)
        
        //self.layoutManager?.addTextContainer(self.textContainer)
        //self.textStorage.?addLayoutManager(self.layoutManager)
        //self.textStorage?.string="Here's to the crazy ones, the misfits, the rebels, the troublemakers, the round pegs in the square holes, the ones who see things differently."
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        /*
         let s=self.textStorage!
         let text:NSAttributedString=NSAttributedString(string: "String", attributes: [NSAttributedStringKey.link:"link"])
         s.insert(text, at: 0)
         s.beginEditing()
